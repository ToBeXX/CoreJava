...
class  NotPrime implements Prime{
...
class JudgeIsPrime{
	public static Prime getIsPrime(int a){
		if(a == 1) return new NotPrime();
		int count = 0;
		for(int i = 2;i<a;i++){
			if(a%i!=0){
				count+=1;
		...
		if(count == a-2) return new IsPrime();
		else return new NotPrime();
	}
}
class TestGdGuess{
	public static void main(String[] args){
	...
	public static void divid(int n){
		for(int num1 = 1;num1<n/2;num1++){
			int num2 = n-num1;
			Prime i = JudgeIsPrime.getIsPrime(num1);
			Prime e = JudgeIsPrime.getIsPrime(num2);
